...
abstract class BasePresenter extends \Nette\Application\UI\Presenter {
  ...
  function tryLogin() {
    ...
    $uid = $this->user->id;
    if($this->presenter instanceof CharacterPresenter AND $uid == -1) return;
    if($this->presenter instanceof CharacterPresenter AND $this->user->identity->stage == NULL) return;
    if($this->presenter instanceof CharacterPresenter AND $uid > 0) $this->redirect(301, "Homepage:default");
    if($this->presenter instanceof IntroPresenter AND $this->user->identity->stage == NULL) return;
    switch($uid) {
case -1:
